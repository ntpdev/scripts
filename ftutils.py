    ...
    for article_list in article_lists:
        for article in article_list.articles:
            if article.url in seen:
                ...
                if len(article.summary) > len(seen[article.url].summary):
                    seen[article.url] = article
            else:
                seen[article.url] = article

    merged = ArticleList(timestamp_retrieved=article_lists[0].timestamp_retrieved, source=article_lists[0].source, articles=list(seen.values()))
    return merged
...
@cache
def ftutils_functions():
    return {
        fn_name(retrieve_headlines_fn): {"defn": retrieve_headlines_fn, "fn": retrieve_headlines},
        fn_name(retrieve_article_fn): {"defn": retrieve_article_fn, "fn": retrieve_article},
        fn_name(retrieve_stock_quotes_fn): {"defn": retrieve_stock_quotes_fn, "fn": retrieve_stock_quotes},
    }
...
def add_exchange_info(ticker: str) -> str:
    t = ticker if ticker.find(":") >= 0 else ticker + ":UN"
    return t.upper()


def process_bnn_stock_page(page, ticker: str) -> dict | None:
    url = f"https://www.bnnbloomberg.ca/stock/{ticker}/"
    console.print("navigating to {url}", style="yellow")
    page.goto(url)

    scorecard = page.locator(".bmw-scorecard.medium")
    if not scorecard:
        return None

    print("scorecard found " + ticker)
    # the table is loaded on demand via a rest call when it is in view
    # alternatively use page.keyboard.press("PageDown")
    scorecard.scroll_into_view_if_needed()
    ...
    d = {}
    d["ticker"] = ticker_head.inner_text()
    ...
    listitems = page.locator("section.bmw-scorecard.medium > ul > li")
...
def parse_nytimes_homepage(page) -> list[ArticleLink]:
...
def retrieve_wsj_home_page() -> ArticleList:
    xs = retrieve_using_playwright("https://www-wsj-com.translate.goog/?_x_tr_sl=auto&_x_tr_tl=en&_x_tr_hl=en&_x_tr_pto=wapp&_x_tr_hist=true",                                        
                                        parse_wsj_homepage)
    return ArticleList(timestamp_retrieved=datetime.now().isoformat(), source="Wall Street Journal", articles=xs)

def retrieve_wsj_home_page_ex() -> ArticleList:
    """retrieve first 10 headlines from wsj mobile home page"""
    xs = []
    with sync_playwright() as p:
...
def retrieve_stock_quotes(tickers: list[str]) -> QuoteList | None:
    ...
    with sync_playwright() as p:
        try:
            browser = p.chromium.launch(headless=False)
            page = browser.new_page()
            xs = [process_bnn_stock_page(page, add_exchange_info(e)) for e in sorted(tickers)]
        except Exception as e:
            console.print(f"error retrieving {e}", style="red")
            return None
        finally:
            browser.close()
    return QuoteList(timestamp_retrieved=datetime.now().isoformat(), quotes=xs)


if __name__ == "__main__":
    # items = retrieve_nyt_home_page()
